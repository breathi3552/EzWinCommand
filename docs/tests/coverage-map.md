...
| 设备会话与撤销 | 已有配对关系无感补充非敏感设备标识；有效会话可在重启和协议升级后恢复；权威设备列表只含 `device_id`、名称、创建时间、最后活动时间和 `is_current`；localhost 全部为 false，远端按鉴权请求标记当前设备；所有管理方按 `device_id` 管理全部设备；撤销成功使请求和活动流失效；撤销失败不误删仍有效的本地会话 | Cross-platform contract + UI/E2E | Automated：Server 公共 HTTP 接口覆盖本机/远端列表、`is_current`、凭据不泄漏、按 `device_id` 重命名/撤销、缺失目标 404、带类型失效事实、媒体只终止目标流、本机重新读取通知和长命令继续执行；Android JVM/Robolectric 覆盖 `is_current` 自身判断、自撤销回配对、同一/不同/不可达 Server identity 的会话处理、授权失效后的媒体重连/迟到回调清理和终止控制页；Web Node 行为测试覆盖 localhost 与鉴权外部页面的列表、重命名、其他设备撤销保持会话、自撤销回配对。Manual — 待人工验证：真实 Keystore/Activity wiring、真实浏览器视觉与断线恢复、跨设备撤销后的媒体流终止。|
| 本地管理安全边界 | 配对码只对 PC 本机开放；设备管理列表与操作可由 localhost 或已鉴权 Paired Device 使用稳定 `device_id`，管理事件不携带敏感值；断线后以权威快照恢复 | Server interface + Web browser | Automated：接口与 Node 行为测试覆盖本机/远端访问、`device_id` 管理、`is_current` 自身/他设备分支和非敏感响应。Manual — 待人工验证：真实浏览器视觉、浏览器断线恢复及多配对卡的人工确认。|
