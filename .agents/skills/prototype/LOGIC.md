...
Once the prototype has answered its question, capture the answer and prototype as [SKILL.md](SKILL.md) describes. The logic-specific mapping: record the validated reducer, machine or function shape as the decision; keep the HTML shell as the rerunnable primary source. Move that source to a throwaway branch only with explicit commit authorization.
