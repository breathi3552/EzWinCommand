...
- Main 根据影响范围、可回滚性、证据质量和失败代价选择验证强度。重需求、协议或安全边界变化依次经过 Test Verifier 和 Goal Reviewer；局部且证据明确的改动可由 Main 自测并按需选择 Reviewer。
...
- 产品与测试资料：优先读取仓库当前已有资料；迁移后使用精简契约、验收覆盖和增量 changelog，历史资料默认不加载
