...
import io.github.ezwincommand.android.network.UnsupportedProtocolException
...
class ConnectionRepository(
    ...
    suspend fun testConnection(baseUrl: String): ConnectionCheckResult {
        ...
        val identity = when (val result = anonymous.identity()) {
            ...
            is ApiResult.HttpError -> {
                if (result.status == 426) return ConnectionCheckResult.Incompatible(protocolMessage())
                return ConnectionCheckResult.Unreachable(connectionMessage(result.status, result.message))
            }
            ...
            is ApiResult.ParseError -> {
                return if (result.isProtocolMismatch()) {
                    ConnectionCheckResult.Incompatible(protocolMessage())
                } else {
                    ConnectionCheckResult.Unreachable("服务器身份无法识别。")
                }
            }
        }
        ...
        if (saved != null && !saved.needsRepair) {
            ...
            if (key != null) {
                ...
                if (authorized is ApiResult.HttpError && authorized.status == 426) {
                    return ConnectionCheckResult.Incompatible(protocolMessage())
                }
        ...
        val created = when (val result = anonymous.createPairing(identity.serverId, "Android")) {
            ...
            is ApiResult.HttpError -> {
                if (result.status == 426) return ConnectionCheckResult.Incompatible(protocolMessage())
                return ConnectionCheckResult.Unreachable(result.message)
            }
            is ApiResult.NetworkError -> return ConnectionCheckResult.Unreachable(networkMessage(normalized, result.message, result.cause))
        ...
        val result = withContext(NonCancellable) {
            withTimeout(PAIRING_COMMIT_TIMEOUT_MILLIS) {
                when (val completed = client.completePairing(current.serverId, current.pairingId, code, resolvedDeviceName)) {
                    ...
                    is ApiResult.HttpError -> if (completed.status == 426) {
                        PairingResult.Failed(protocolMessage(), false)
                    } else {
                        PairingResult.Failed(completed.message.ifBlank { "验证码无效或已锁定。" }, completed.status == 401 || completed.status == 403)
                    }
    ...
    suspend fun restoreSession(): RestoreResult {
        ...
        val anonymous = clientFactory(target.baseUrl) { null }
        when (val identityResult = anonymous.identity()) {
            is ApiResult.Success -> {
                if (!isCompatible(identityResult.value)) {
                    return RestoreResult.InvalidSavedSession(target.serverId, protocolMessage())
                }
                if (identityResult.value.serverId != target.serverId) {
                    return RestoreResult.InvalidSavedSession(target.serverId, "服务器身份已变化，请重新选择服务。")
                }
            }
            is ApiResult.HttpError -> {
                val message = if (identityResult.status == 426) protocolMessage()
                else connectionMessage(identityResult.status, identityResult.message)
                return RestoreResult.InvalidSavedSession(target.serverId, message)
            }
            is ApiResult.NetworkError -> return RestoreResult.InvalidSavedSession(target.serverId, networkMessage(target.baseUrl, identityResult.message, identityResult.cause))
            is ApiResult.ParseError -> {
                val message = if (identityResult.isProtocolMismatch()) protocolMessage() else "服务器身份无法识别。"
                return RestoreResult.InvalidSavedSession(target.serverId, message)
            }
        }
        val key = sessions.readDeviceKey(target.serverId)
            ?: return RestoreResult.InvalidSavedSession(target.serverId, "会话需要重新配对。")
    ...
    suspend fun migrateLegacy(): Boolean {
        ...
        val identity = when (val result = clientFactory(normalized) { null }.identity()) {
            is ApiResult.Success -> {
                if (!isCompatible(result.value)) return false
                result.value
            }
            else -> return false
        }
    ...
    fun normalizeInputForAndroid(address: String, port: String?): NormalizeResult {
        ...
        val candidate = if (rawAddress.contains("://")) {
            appendPortIfMissing(rawAddress, rawPort)
        } else if (rawPort.isEmpty() || rawAddress.substringAfterLast('/').contains(':')) {
            rawAddress
        } else {
            "$rawAddress:$rawPort"
        }
    ...
    private fun appendPortIfMissing(address: String, port: String): String {
        ...
        val uri = runCatching { URI(address) }.getOrNull() ?: return address
        return if (uri.port != -1 || uri.host.isNullOrBlank()) address else "${uri.scheme}://${uri.host}:$port"
    }
    ...
    private fun normalizeBaseUrl(value: String): String? {
        ...
        val uri = runCatching { URI(if (raw.contains("://")) raw else "http://$raw") }.getOrNull()
            ?: return fail("服务地址格式不正确。")
        val scheme = uri.scheme?.lowercase()
        val host = uri.host?.trim().orEmpty()
        if (scheme !in setOf("http", "https") || host.isEmpty()) return fail("服务地址格式不正确。")
        if (host in setOf("0.0.0.0", "localhost", "127.0.0.1")) return fail("请输入电脑的局域网地址。")
        return "$scheme://$host${if (uri.port == -1) "" else ":${uri.port}"}"
    }
    ...
    private fun isCompatible(identity: io.github.ezwincommand.android.model.ServerIdentity): Boolean =
        identity.protocolVersion == AppConstants.PROTOCOL_VERSION

    private fun protocolMessage() = "服务器协议不兼容，请升级应用。"

    private fun ApiResult<*>.isProtocolMismatch(): Boolean =
        this is ApiResult.ParseError && cause is UnsupportedProtocolException

    private fun connectionMessage(status: Int, message: String) =
        if (status == 401 || status == 403) "会话已失效，请重新配对。" else message.ifBlank { "无法连接到服务。" }
